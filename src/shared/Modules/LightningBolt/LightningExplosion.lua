...
function LightningExplosion.new(Position, Size, NumBolts, Color, BoltColor, UpVector)
	...
	for i = 1, self.NumBolts do
		...
		NewBolt.Sparks = LightningSparks.new(NewBolt, 5)
		NewBolt.Sparks.MinDistance, NewBolt.Sparks.MaxDistance = 7.5, 10
...
function LightningExplosion:Destroy()
	...
	for i = 1, #self.Bolts do
		if (self.Bolts[i].Sparks) then
			self.Bolts[i].Sparks:Destroy()
		end
